...
class DashBoard extends Component {
  ...
  async getAPY() {
    const vaultAPYdata = await axios.get(apyEndPoints.savings);
    const nordAPYdata = await axios.get(apyEndPoints.nord);
    const vault_apy = [];
    const nord_apy = [];
  ...
  async updateNordBalance() {
    if (this.state.accounts) {
      let bal = 0;
      let unclaimBal = 0;
      const nordGovernanceContract = new this.web3.eth.Contract(
        nordGovernanceData[0].nordABI.abi,
        nordGovernanceData[0].contract
      );
      await nordGovernanceContract.methods
        .balanceOf(this.state.accounts[0])
        .call({ from: this.state.accounts[0] }, function (error, result) {
          if (error) {
            console.log(`Error while fetching Nord Token balance: ${error}`);
            return;
          }
          bal = result;
        });
      bal = this.web3.utils.fromWei(
        bal,
      ...
      await claimContract.methods
        ...
        .call({ from: this.state.accounts[0] }, function (error, res) {
          if (error) {
            console.log(
              `Error while fetching Nord Token balance for claim: ${error}`
            );
            return;
          }
      ...
      unclaimBal = this.web3.utils.fromWei(
        unclaimBal,
        nordGovernanceData[0].web3EquivalentPrecision
      );
      this.setState(() => {
        return {
          nTokenBalance: [bal, unclaimBal],
        };
      });
    } else {
      this.logOut();
  ...
  async updateBalance() {
    ...
    if (this.state.accounts) {
      ...
      const tokenName = [];
      let tSupply = 0;
      const usdPrice = await axios.get(coingeckoPriceEndPoint);
      ...
      for (const x of vaultData) {
        ...
        vbal.push(this.web3.utils.fromWei(res, x.web3EquivalentPrecision));
        tSupply +=
          (res * usdPrice.data[x.priceApiName].usd) / Math.pow(10, x.precision);
        await vaultContract.methods
          ...
          .call({ from: this.state.accounts[0] }, function (error, result) {
            ...
            if (error) {
              console.log(
                `Error while fetchng getPricePerFullShare for vault: ${x.name} : ${error}`
              );
            } else {
              pricePerFullShare = result;
            }
            share.push(pricePerFullShare);
          });
        ...
        await ercContract.methods
          ...
          .call({ from: this.state.accounts[0] }, function (error, result) {
            ...
            if (error) {
              console.log(
                `Error occured while fetching token balance for ${x.name} : ${error}`
              );
            } else {
              balance = web3Instance.utils.toBN(result);
            }
            bal.push(balance);
          });

        const nordContract = new this.web3.eth.Contract(
          x.vaultABI.abi,
          x.contract
        );
        await nordContract.methods
          .balanceOf(this.state.accounts[0])
          .call(
            { from: this.state.accounts[0] },
            function (error, nordBalance) {
              let balance = 0;
              if (error) {
                console.log(
                  `Error occured while fetching token balance for nordToken : ${error}`
                );
              } else {
                balance = web3Instance.utils.toBN(nordBalance);
              }
              nbal.push(balance);
            }
          );
      ...
      await this.updateNordBalance();
      await this.getAPY();
      const interval = setTimeout(this.updateBalance, 120000);
      this.setState(() => {
        return {
          isLoading: false,
